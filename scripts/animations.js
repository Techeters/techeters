...
const animateOut = $el => {
  ...
  if ($hl.length) {
    $hl.forEach(async (el, i) => {
      await delayPromise(150 * i)
      TextAnimate.out(el, 'lines')
    })
  }

  $t.length &&
    tl.to($t, { duration: 1.5, y: 60, opacity: 1, ease, stagger: 0.2 }, 0.5)

  $l.length && tl.to($l, { duration: 1.5, scaleX: 0, ease, stagger: 0.2 }, 0)

  $o.length && tl.to($o, { duration: 1.5, opacity: 0, ease, stagger: 0.2 }, 0.5)
